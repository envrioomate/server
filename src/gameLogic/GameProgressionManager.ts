...
export class GameProgressionManager implements EntitySubscriberInterface {
    ...
    public async getCurrentlySelectedAchievementsForUser(user: User) {
        const currentlySelectedCutoffTime: Date = moment().subtract(12, "weeks").toDate(); // get Achievements from the last few month ~ average completion duration
        return this.achievementSelectionRepository.find({
            where: [
                {owner: user, createdAt: MoreThan(currentlySelectedCutoffTime)}, // OR
            ]
        });
    }
    ...
    public async getPastSelectedAchievementsForUser(user: User) {
        ...
        return this.achievementSelectionRepository.find({
            where: {
                owner: user,
                createdAt: LessThan(currentlySelectedCutoffTime)
            }
        });
    }
